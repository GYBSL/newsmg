  ...
  >
    <div class="mustSee">
      <div class="libox" ref="libox">
        <template v-for="(item, index) in mustSeeArr ? mustSeeArr : ''">
          <HomeCardTitle
...
export default {
  ...
  methods: {
    getCareData() {
      this.axios
        .get(`https://apis.netstart.cn/xpc/home/mustSee`)
        .then((res) => {
          this.$store.commit("changemustSeeObj", res.data.data);
          this.$store.commit("changemustSeeArr", res.data.data.children);
    ...
    getCareVideo() {
      if (this.$route.path == "/home/mustSee") {
        if (this.sendTime) {
